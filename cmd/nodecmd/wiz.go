...
func wiz(cmd *cobra.Command, args []string) error {
	...
	if subnetName != "" && (!app.SidecarExists(subnetName) || forceSubnetCreate) {
		...
		if err := subnetcmd.CallCreate(
			cmd,
			subnetName,
			subnetcmd.CreateFlags{
				ForceCreate:                    forceSubnetCreate,
				GenesisFile:                    subnetGenesisFile,
				UseSubnetEvm:                   useEvmSubnet,
				UseCustom:                      useCustomSubnet,
				EvmVersion:                     evmVersion,
				EvmChainID:                     evmChainID,
				EvmToken:                       evmToken,
				EvmDefaults:                    evmDefaults,
				UseLatestReleasedEvmVersion:    useLatestEvmReleasedVersion,
				UseLatestPreReleasedEvmVersion: useLatestEvmPreReleasedVersion,
